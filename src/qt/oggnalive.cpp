...
#include <config/oggnalive-config.h>
#endif

#include <qt/oggnalive.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<oggnaliveUnit>("oggnaliveUnit");
#else
    qRegisterMetaType<oggnaliveUnit>("oggnaliveUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. oggnalive_de.qm (shortcut "de" needs to be defined in oggnalive.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. oggnalive_de_DE.qm (shortcut "de_DE" needs to be defined in oggnalive.qrc)
...
static const char* qt_argv = "oggnalive-qt";

oggnaliveApplication::oggnaliveApplication()
...
void oggnaliveApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the oggnaliveApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", oggnaliveGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void oggnaliveApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool oggnaliveApplication::createOptionsModel(bool resetSettings)
{
...
void oggnaliveApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new oggnaliveGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &oggnaliveGUI::quitRequested, this, &oggnaliveApplication::requestShutdown);
...
void oggnaliveApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void oggnaliveApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool oggnaliveApplication::baseInitialize()
{
    return node().baseInitialize();
}

void oggnaliveApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &oggnaliveApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &oggnaliveApplication::handleRunawayException);
    connect(this, &oggnaliveApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &oggnaliveApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void oggnaliveApplication::parameterSetup()
{
...
void oggnaliveApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void oggnaliveApplication::requestInitialize()
{
...
void oggnaliveApplication::requestShutdown()
{
...
void oggnaliveApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // oggnalive: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &oggnaliveGUI::handlePaymentRequest);
            connect(window, &oggnaliveGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void oggnaliveApplication::handleRunawayException(const QString &message)
{
...
void oggnaliveApplication::handleNonFatalException(const QString& message)
{
...
WId oggnaliveApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool oggnaliveApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", oggnaliveGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(oggnalive);
    Q_INIT_RESOURCE(oggnalive_locale);
    ...
    /// 6-7. Parse oggnalive.conf, determine network, switch to network specific
    ...
    // oggnalive: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
