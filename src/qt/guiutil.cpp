...
#include <qt/oggnaliveaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a oggnalive address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new oggnaliveAddressEntryValidator(parent));
    widget->setCheckValidator(new oggnaliveAddressCheckValidator(parent));
}
...
bool parseoggnaliveURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no oggnalive: URI
    if(!uri.isValid() || uri.scheme() != QString("oggnalive"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!oggnaliveUnits::parse(oggnaliveUnit::OGG, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseoggnaliveURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseoggnaliveURI(uriInstance, out);
}

QString formatoggnaliveURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("oggnalive:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(oggnaliveUnits::format(oggnaliveUnit::OGG, info.amount, false, oggnaliveUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openoggnaliveConf()
{
    ...
    /* Open oggnalive.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "oggnalive.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "oggnalive (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("oggnalive (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for oggnalive*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "oggnalive.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("oggnalive-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a oggnalive.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=oggnalive\n";
        else
            optionFile << strprintf("Name=oggnalive (%s)\n", chain);
