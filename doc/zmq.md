...
The oggnalive Core daemon can be configured to act as a trusted "border
router", implementing the oggnalive wire protocol and relay, making
...
The ZeroMQ feature in oggnalive Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building oggnalived:
...
For instance:
    ...
    $ oggnalived -zmqpubhashtx=tcp://127.0.0.1:213369 \
               -zmqpubhashtx=tcp://192.168.1.2:213369 \
               -zmqpubhashblock="tcp://[::1]:213370" \
               -zmqpubrawtx=ipc:///tmp/oggnalived.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in oggnalive.conf.
...
From the perspective of oggnalived, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into oggnalived directly. Furthermore, no information is
...
using. oggnalived appends an up-counting sequence number to each
