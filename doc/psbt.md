# PSBT Howto for oggnalive Core

Since oggnalive Core 0.17, an RPC interface exists for Partially Signed oggnalive
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/oggnalive/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for oggnalive transactions that are not fully signed
...
#### Multisig with multiple oggnalive Core instances
...
oggnalive Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `oggnalive-cli` is used.
...
Later, when *V* OGG has been received on *Amulti*, and Bob and Carol want to
