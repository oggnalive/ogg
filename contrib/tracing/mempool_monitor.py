...
""" Example logging oggnalive Core mempool events using the mempool:added,
...
def main(oggnalived_path):
    oggnalived_with_usdts = USDT(path=str(oggnalived_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    oggnalived_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    oggnalived_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    oggnalived_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    oggnalived_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[oggnalived_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/oggnalived")
        exit(1)
