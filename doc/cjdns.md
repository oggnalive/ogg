# CJDNS support in oggnalive Core

It is possible to run oggnalive Core over CJDNS, an encrypted IPv6 network that
...
done outside of oggnalive Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the oggnalive network and individual nodes.
...
before it will work with your oggnalive Core node. This is described in steps
...
## Run oggnalive Core with CJDNS

Once you are connected to the CJDNS network, the following oggnalive Core
...
When enabled, this option tells oggnalive Core that it is running in an
...
IPv6 local network. This helps oggnalive Core perform better address management:
...
CJDNS support was added to oggnalive Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `oggnalive-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/oggnalive/oggnalive/blob/master/doc/tor.md) for
details.

## CJDNS-related information in oggnalive Core

There are several ways to see your CJDNS address in oggnalive Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `oggnalive-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `oggnalive-cli getpeerinfo`).
