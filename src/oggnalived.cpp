...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/oggnalive.conf are parsed in qt/oggnalive.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  oggnalived [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see oggnalived -h for a list of options.", argv[i])));
            }
        }

        // -server defaults to true for oggnalived but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect oggnalived signal handlers
    noui_connect();
