# Bootstrappable oggnalive Core Builds

This directory contains the files necessary to perform bootstrappable oggnalive
Core builds.
...
To build oggnalive Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      oggnalive Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/oggnalive-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/oggnalive-core/guix.sigs).
