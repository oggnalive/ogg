# User-space, Statically Defined Tracing (USDT) for oggnalive Core

oggnalive Core includes statically defined tracepoints to allow for more
...
**Note**: oggnalive Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to oggnalive Core
...
Multiple tools can list the available tracepoints in a `oggnalived` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in oggnalive Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/oggnalived
stap net        outbound_message 0x0000000000107c05 /src/oggnalived
stap validation block_connected  0x00000000002fb10c /src/oggnalived
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in oggnalive Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/oggnalived | grep NT_STAPSDT -A 4 -B 2
