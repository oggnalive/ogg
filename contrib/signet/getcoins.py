...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing oggnalive-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='oggnalive-cli', help='oggnalive-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='oggnalive address to which the faucet should send')
...
parser.add_argument('oggnalive_cli_args', nargs='*', help='Arguments to pass on to oggnalive-cli (default: -signet)')

args = parser.parse_args()

if args.oggnalive_cli_args == []:
    args.oggnalive_cli_args = ['-signet']


def oggnalive_cli(rpc_command_and_params):
    argv = [args.cmd] + args.oggnalive_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = oggnalive_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = oggnalive_cli(['getnewaddress', 'faucet', 'bech32'])
