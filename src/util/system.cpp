...
const char * const OGGNALIVE_CONF_FILENAME = "oggnalive.conf";
const char * const OGGNALIVE_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //oggnalive-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", OGGNALIVE_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\oggnalive
    // macOS: ~/Library/Application Support/oggnalive
    // Unix-like: ~/.oggnalive
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "oggnalive";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/oggnalive";
#else
    // Unix-like
    return pathRet / ".oggnalive";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", OGGNALIVE_CONF_FILENAME));
}
