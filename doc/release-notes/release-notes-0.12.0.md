oggnalive Core version 0.12.0 is now available from:

  <https://oggnalive.org/bin/oggnalive-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/oggnalive-Qt (on Mac) or
oggnalived/oggnalive-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of oggnalive Core or other software:
...
pre-0.12 versions of oggnalive Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start oggnalive Core version 0.11 or
earlier.
...
ECDSA signatures inside oggnalive transactions now use validation using
[libsecp256k1](https://github.com/oggnalive-core/secp256k1) instead of OpenSSL.
...
(https://github.com/oggnalive/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of oggnalive Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of oggnalive Core was to raise the default minimum
relay fee.

oggnalive Core 0.12 will have a strict maximum size on the mempool. The
...
oggnalive Core 0.12 also introduces new default policy limits on the length and
...
oggnalive Core 0.12 nodes. oggnalive Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/oggnalive/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in oggnalive Core 0.12 does not yet have support for
...
This allows running oggnalived without having to do any manual configuration.
...
oggnalive Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. oggnalive Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In oggnalive Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in oggnalive Core 0.13, and it is at this time undecided whether the
...
oggnalive Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
oggnalive Core automatically creates a hidden service to listen on, without
manual configuration. oggnalive Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if oggnalive Core is listening, and
...
oggnalived can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals oggnalive
Core to use floating fees. By default, oggnalive Core will use floating
fees.
...
(default: `0.0002` OGG/kB).

At all times, oggnalive Core will cap fees at `-maxtxfee=<x>` (default:
0.10) OGG.
Furthermore, oggnalive Core will never create transactions paying less than
...
`oggnalive.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/oggnalive/oggnalive/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/oggnalive/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/oggnalive/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 213369 to a RPC server bound on localhost on port 23369 do:

    stunnel -d 213369 -r 127.0.0.1:23369 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /oggnaliverpc>
        ProxyPass http://127.0.0.1:13369/
        ProxyPassReverse http://127.0.0.1:13369/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass oggnalived rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< oggnaliverpc:<password>
    </Location>
...
Restarting oggnalived will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of oggnalive-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new oggnalive-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include oggnalive-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix oggnalive-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of oggnalive-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override OGGNALIVED and OGGNALIVECLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [oggnalive-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add OGGNALIVED_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library liboggnaliveconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "OGG" (Ross Nicoll)
