...
/** Call node.updateRwSetting() with oggnalive 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because oggnalive 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/oggnalive/oggnalive/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because oggnalive 22.x will not try to read these.
...
//! Convert enabled/size values to oggnalive -prune setting.
...
//! Get pruning enabled value to show in GUI from oggnalive -prune setting.
...
//! Get pruning size value to show in GUI from oggnalive -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayoggnaliveUnit")) {
        settings.setValue("DisplayoggnaliveUnit", QVariant::fromValue(oggnaliveUnit::OGG));
    }
    QVariant unit = settings.value("DisplayoggnaliveUnit");
    if (unit.canConvert<oggnaliveUnit>()) {
        m_display_oggnalive_unit = unit.value<oggnaliveUnit>();
    } else {
        m_display_oggnalive_unit = oggnaliveUnit::OGG;
        settings.setValue("DisplayoggnaliveUnit", QVariant::fromValue(m_display_oggnalive_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if oggnalive.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_oggnalive_unit);
...
{
    if (new_unit.isNull() || new_unit.value<oggnaliveUnit>() == m_display_oggnalive_unit) return;
    m_display_oggnalive_unit = new_unit.value<oggnaliveUnit>();
    QSettings settings;
    settings.setValue("DisplayoggnaliveUnit", QVariant::fromValue(m_display_oggnalive_unit));
    Q_EMIT displayUnitChanged(m_display_oggnalive_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/oggnalive/oggnalive/pull/8273
    ...
    // (https://github.com/oggnalive-core/gui/issues/567).
