...
TMPDIR_PREFIX = "oggnalive_func_test_"
...
class oggnaliveTestMetaClass(type):
    """Metaclass for oggnaliveTestFramework.

    Ensures that any attempt to register a subclass of `oggnaliveTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'oggnaliveTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("oggnaliveTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("oggnaliveTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class oggnaliveTestFramework(metaclass=oggnaliveTestMetaClass):
    """Base class for a oggnalive test script.

    Individual oggnalive test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave oggnaliveds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop oggnaliveds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use oggnalive-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_oggnalived = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "oggnalived" + config["environment"]["EXEEXT"],
        )
        fname_oggnalivecli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "oggnalive-cli" + config["environment"]["EXEEXT"],
        )
        fname_oggnaliveutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "oggnalive-util" + config["environment"]["EXEEXT"],
        )
        self.options.oggnalived = os.getenv("OGGNALIVED", default=fname_oggnalived)
        self.options.oggnalivecli = os.getenv("OGGNALIVECLI", default=fname_oggnalivecli)
        self.options.oggnaliveutil = os.getenv("OGGNALIVEUTIL", default=fname_oggnaliveutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: oggnaliveds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("oggnaliveRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'oggnalived', self.options.oggnalived) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'oggnalive-cli', self.options.oggnalivecli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a oggnalived test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple oggnalived test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as oggnalived's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    oggnalived=self.options.oggnalived,
                    oggnalive_cli=self.options.oggnalivecli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in oggnalive.conf
    ...
    def skip_if_no_oggnalived_tracepoints(self):
        """Skip the running test if oggnalived has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("oggnalived has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_oggnalived_zmq(self):
        """Skip the running test if oggnalived has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("oggnalived has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if oggnalive-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("oggnalive-wallet has not been compiled")

    def skip_if_no_oggnalive_util(self):
        """Skip the running test if oggnalive-util has not been compiled."""
        if not self.is_oggnalive_util_compiled():
            raise SkipTest("oggnalive-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if oggnalive-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("oggnalive-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether oggnalive-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether oggnalive-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_oggnalive_util_compiled(self):
        """Checks whether oggnalive-util was compiled."""
        return self.config["components"].getboolean("ENABLE_OGGNALIVE_UTIL")
