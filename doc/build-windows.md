...
Below are some notes on how to build oggnalive Core for Windows.

The options known to work for building oggnalive Core on Windows are:
...
Note that for WSL the oggnalive Core source path MUST be somewhere in the default mount file system, for
example /usr/src/oggnalive, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\oggnalive`, for example:

    make install DESTDIR=/mnt/c/workspace/oggnalive
