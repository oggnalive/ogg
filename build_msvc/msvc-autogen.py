...
def parse_config_into_ogg_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"oggnalive Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/oggnalive_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/oggnalive_config.h'), "w", encoding="utf8") as ogg_config:
        ogg_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='oggnalive-core msbuild configuration initialiser.')
    ...
    parse_config_into_ogg_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/oggnalive_config.h'), os.path.join(SOURCE_DIR, 'config/oggnalive-config.h'))
